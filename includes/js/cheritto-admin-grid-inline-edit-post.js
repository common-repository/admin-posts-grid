...
( function( $, wp ) {
	...
	edit : function(id) {
		...
		$(t.what+id).after(editRow).after('<tr class="hidden"></tr>');

		editRow.appendTo($("#inlinemodal"));
		$("#inlinemodal").show();
	...
	save : function(id) {
		...
		$( '.inline-edit-save .spinner' ).addClass( 'is-active' );

		$( '.inline-edit-save .cancel' ).remove();

		$( '.inline-edit-save .save' ).prop('disabled',true);

		$( '.inline-edit-save .save' ).text("Saving...");
		...
		$.post( ajaxurl, params,
			function(r) {
				...
				$( '.inline-edit-save .spinner' ).removeClass( 'is-active' );

				if (r) {
					if ( -1 !== r.indexOf( '<tr' ) ) {
						$( '.inline-edit-save .save' ).text("SAVED!");
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							$("#post-"+id+" .editinline").css("color",'');
							$("#post-"+id+" .editinline").prop("disabled",false);
							//$("#inlinemodal").hide();
	 ...
	 */
	getId : function(o,selector='.cheritto-admin-grid-item') {
		var id = $(o).closest(selector).attr('id'),
			parts = id.split('-');
