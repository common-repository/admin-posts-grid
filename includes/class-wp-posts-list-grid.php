...
class WP_Posts_List_Grid extends WP_List_Grid {
	...
	public function prepare_items() {
        ...
        // Necessary for this reason: https://wordpress.stackexchange.com/questions/67669/how-to-stop-wpnonce-and-wp-http-referer-from-appearing-in-url
	...
	protected function get_bulk_actions() {
		...
		if ( current_user_can( $post_type_obj->cap->edit_posts ) ) {
			...
			}*/ // Bulk edit currently not supported
		}
	...
	protected function formats_dropdown( $post_type ) {
		...
		$displayed_post_format = isset( $_GET['post_format'] ) ? sanitize_text_field($_GET['post_format']) : '';
		?>
	...
	public function get_columns() {
        ...
        $posts_columns['image'] = __( 'Image' );

		/* translators: Posts screen column name. */
		$posts_columns['title'] = _x( 'Title', 'column name' );

		$posts_columns['actions'] = __( 'Actions' );
	...
	public function column_cb( $item ) {
		...
		if ( $can_edit_post && 'trash' !== $post->post_status ) {
			...
			if ( $lock_holder ) {
				...
				$locked_text = esc_html( sprintf( __( '%s editing' ), $lock_holder->display_name ) );
			} else {
				$locked_avatar = '';
				$locked_text   = '';
			}
		...
		if ( apply_filters( 'wp_list_table_show_post_checkbox', $show, $post ) ) :
			...
			<div>
				<div style="float:left;width:10%">
					<input class="cheritto-admin-grid-item-checkbox" id="cb-select-<?php the_ID(); ?>" type="checkbox" name="post[]" value="<?php the_ID(); ?>" />
				</div>
				<div class="locked-indicator" style="float:right;width:85%;text-align:right;">
					
					<?php
						echo '<div class="locked-info" style="margin-top:0;"><span style="margin-right:3px;" class="locked-indicator-icon" aria-hidden="true"></span><span class="locked-avatar">' . $locked_avatar . '</span> <span class="locked-text" style="font-size:0.8em">' . esc_html($locked_text) . "</span></div>\n";
					?>
	...
	protected function _column_title( $post, $classes, $data, $primary ) {
        ...
        echo '<div class="cheritto-admin-grid-item-line-limiter limit-2">';
		$allowed = [ 'strong' => [],
					'a' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]] 
		];
		echo wp_kses($this->column_title( $post ),$allowed);
        echo '</div>';
		//echo '</td>';
	}

	protected function _column_actions( $post, $classes, $data, $primary ) {
		//echo '<td class="' . $classes . ' page-title" ', $data, '>';
        echo '<div class="cheritto-admin-grid-item-line-limiter limit-2">';
		$allowed = [ 'div' => ['class'=>[]], 
					'span' => ['class'=>[]], 
					'a' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]],
					'button' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]]  
		];
		echo wp_kses($this->handle_row_actions( $post, 'title', $primary ),$allowed);
        echo '</div>';
		//echo '</td>';
	}
	 ...
	 * @param string  $primary
	 */
	protected function _column_categories( $post, $classes, $data, $primary ) {
        echo '<div class="cheritto-admin-grid-item-line-limiter limit-2">'.__('Categories: ');
		$allowed = [ 'strong' => [],
					'a' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]] 
		];
		echo wp_kses($this->column_default( $post, 'categories' ),$allowed);
        echo '</div>';
	}
	 ...
	 * @param string  $primary
	 */
	protected function _column_tags( $post, $classes, $data, $primary ) {
       
		echo '<div class="cheritto-admin-grid-item-line-limiter limit-2">'.__('Tags: ');
		$allowed = [ 'strong' => [],
					'a' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]] 
		];
		echo wp_kses($this->column_default( $post, 'tags' ),$allowed);
        echo '</div>';
	}
    ...
    protected function _column_image( $post, $classes, $data, $primary ) {
		...
		echo '<div class="cheritto-admin-grid-thumbnail" style="background-image:url(\''.esc_attr($featured_img_url).'\');"></div>';
	...
	public function column_author( $post ) {
        ...
        echo '<div class="cheritto-admin-grid-item-line-limiter limit-1">'.__('Author: ');
		$allowed = [ 'strong' => [],
					'a' => ['class'=>[],'href'=>[],'aria-label'=>[],'aria-expanded'=>[]] 
		];
		echo wp_kses($this->get_edit_link( $args, get_the_author() ),$allowed);
        echo '</div>';
	}
		...
		if ( $taxonomy ) {
			...
			if ( is_array( $terms ) ) {
				...
				foreach ( $terms as $t ) {
					...
					$link = $this->get_edit_link( $posts_in_term_qv, $label );

					$link = str_replace("?","&",$link);
					$link = str_replace("edit.php","admin.php?page=admin-posts-grid%2Fincludes%2Fwp-admin%2Fedit.php",$link);
				...
				echo wp_kses(implode( wp_get_list_item_separator(), $term_links ),$allowed);
			} else {
	 ...
	 * @param int         $level
	 */
	public function single_row( $post, $level = 0, $i = -1 ) {
		
        $user = wp_get_current_user();

		$column_count = (int) get_option('cheritto-admin-grid-columns-count-' . $user->user_nicename);

		if ($column_count==0) $column_count=6;

		$per_page = (int) get_option('cheritto-admin-grid-per-page-' . $user->user_nicename);

		if ($column_count==0) $column_count=12;

		if ((($i+1) % $column_count) == 1) $_add_class="first";
		elseif ( (($i+1) % $column_count) == 0 || (($i+1) % $per_page) == 0 ) $_add_class="last";
		else $_add_class="";
		...
		$item_bg = get_option('cheritto-admin-grid-theme-' . $user->user_nicename);

		if (!$item_bg || $item_bg=='none') $item_bg_class = 'cheritto-admin-grid-bg-white';
		else $item_bg_class = 'cheritto-admin-grid-item-bg-' . $item_bg;

		?>

        <div id="post-<?php echo esc_attr($post->ID); ?>" class="<?php echo esc_attr($item_bg_class); ?> <?php echo esc_attr($_add_class); ?> cheritto-admin-grid-item cheritto-admin-grid-item-<?php echo (int) get_option('cheritto-admin-grid-columns-count-' . $user->user_nicename) ?>-columns <?php echo esc_attr(implode( ' ', get_post_class( $classes, $post->ID ) )); ?>">
		...
		while ( $bulk < 2 ) :
			...
			<fieldset class="inline-edit-col-right">
				...
				if ( post_type_supports( $screen->post_type, 'author' ) && $bulk ) {
					echo esc_html($authors_dropdown);
				}
				?>
				...
				<?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?>
					...
					<?php if ( ! $bulk ) : ?>
						...
						<label>
							...
							<span class="input-text-wrap"><input type="text" name="menu_order" class="inline-edit-menu-order-input" value="<?php echo esc_attr($post->menu_order); ?>" /></span>
						</label>
					...
					<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
						...
						<?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) : ?>
							...
							<label class="inline-edit-tags">
								...
								<textarea data-wp-taxonomy="<?php echo esc_attr($taxonomy_name); ?>" cols="22" rows="1" name="tax_input[<?php echo esc_attr( $taxonomy->name ); ?>]" class="tax_input_<?php echo esc_attr( $taxonomy->name ); ?>" aria-describedby="inline-edit-<?php echo esc_attr( $taxonomy->name ); ?>-desc"></textarea>
							</label>
				...
				<?php if ( $bulk && current_theme_supports( 'post-formats' ) && post_type_supports( $screen->post_type, 'post-formats' ) ) : ?>
					...
					<label class="alignleft">
						...
						<select name="post_format">
							<option value="-1"><?php _e( '&mdash; No Change &mdash;' ); ?></option>
							<option value="0"><?php echo esc_html(get_post_format_string( 'standard' )); ?></option>
